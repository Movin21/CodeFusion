...
function Student_Resume() {
    ...
    const fetchResume = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/Resume/getResume",
          {
            headers: {
    ...
    if (file) {
      ...
      try {
        const response = await axios.put(
          "http://localhost:5000/Resume/updateResume",
          formData,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const handleDownload = async () => {
    if (savedResume) {
      try {
        const response = await axios.get(
          `http://localhost:5000/Resume/getResume/${savedResume}`,
          {
            responseType: "blob",
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
          ...
          <ModalBody>
            <VStack spacing={4} align="stretch">
              ...
              >
                ...
                <Text fontSize="sm" className="font-poppins">
                  OR
                </Text>
                <Input
                  type="file"
              ...
              >
                <option value="onlyMe" className="font-poppins">
                  Only Me
                </option>
                <option value="public" className="font-poppins">
                  Public
                </option>
              </Select>
            </VStack>
